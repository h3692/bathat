...
void composite_place(const Nv12Frame& src, Nv12Dest& dst, int x, int y) {
    if (!src.y || !src.uv || !dst.y || !dst.uv) return;
    x &= ~1;                            // even offsets keep U,V pairs aligned
    y &= ~1;                            // ...and Y rows paired with UV rows
    ...
    for (int row = 0; row < h; ++row) {
        std::memcpy(dst.y + static_cast<size_t>(y + row) * dst.y_stride + x,
                    src.y + static_cast<size_t>(row) * src.y_stride,
                    static_cast<size_t>(w));
    }

    // UV plane: h/2 rows starting at dst row y/2. NV12 chroma is half-height
    // but full-width in bytes (each row holds w/2 interleaved U,V pairs =
    // w bytes), so the horizontal offset and copy width match the Y plane.
    const int uv_rows = h / 2;
    for (int row = 0; row < uv_rows; ++row) {
        std::memcpy(dst.uv + static_cast<size_t>(y / 2 + row) * dst.uv_stride + x,
